...
@pytest.mark.asyncio
async def test_full_import_with_realistic_beerxml(test_db: AsyncSession):
    """
    ...
    recipe_id = await import_beerxml_to_db(test_db, xml_content)
...
@pytest.mark.asyncio
async def test_api_endpoint_returns_full_recipe(test_db: AsyncSession):
    """
    Test that fetching an imported recipe returns all data correctly.
    """
    ...
    # Fetch recipe with all relationships (simulates API query)
    result = await test_db.execute(
        select(Recipe)
        .where(Recipe.id == recipe_id)
        .options(
    ...
    assert len(recipe.cultures) == 2

    # Verify fermentable data structure
    assert recipe.fermentables[0].name == "Pale Malt 2-Row"
    assert recipe.fermentables[0].amount_kg == 5.5
    ...
    us05 = next(y for y in recipe.cultures if y.product_id == "US-05")
    assert us05.lab == "Fermentis"
    assert us05.attenuation_percent == 81.0
