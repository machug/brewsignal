...
async def test_migration_preserves_existing_hop_rows():
    ...
    try:
        ...
        # Always clean up the fixture row so the shared SQLite db is left
        # row-clean even when assertions above fail. The schema change is
        # idempotent and benign; the sentinel row at id=9999 is not.
        async with engine.begin() as conn:
            await conn.execute(text("DELETE FROM recipe_hops WHERE id = 9999"))
